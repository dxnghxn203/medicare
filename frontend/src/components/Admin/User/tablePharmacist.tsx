...
const AdminTable = ({
  ...
  const { fetchUpdateStatusPharmacist, fetchAllPharmacist, allPharmacist } =
    useUser();
  ...
  const handleToggleStatus = (pharmacist: any) => {
    const newStatus = !pharmacist.active;
    fetchUpdateStatusPharmacist(
      { pharmacist_id: pharmacist._id, status_pharmacist: newStatus },
      () => {
  ...
  const totalPages = Math.ceil(totalPharmacists / pageSize);
      ...
      <div className="overflow-x-auto">
        <table className="divide-y divide-gray-200 w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {pharmacists.map((pharmacists) => (
              <tr key={pharmacists._id} className="hover:bg-gray-50 relative">
                <td className="px-2 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    ...
                    <button
                      onClick={() => copyToClipboard(pharmacists._id)}
                      className="ml-2 text-gray-400 hover:text-blue-500"
                    >
                      {copiedId === pharmacists._id ? (
                ...
                <td className="px-2 py-4 whitespace-nowrap">
                  <span
                    className={`px-2 py-1 inline-flex text-xs font-semibold rounded-full ${
                      pharmacists.auth_provider === "email"
                        ? "bg-blue-100 text-blue-800"
                        : "bg-purple-100 text-purple-800"
                    }`}
                  >
                    {pharmacists.auth_provider === "email" ? "Email" : "Google"}
                  </span>
                </td>
                <td className="px-2 py-4 whitespace-nowrap text-center">
                  <button
                    onClick={() => handleToggleStatus(pharmacists)}
                ...
                <td className="px-2 py-4 whitespace-nowrap text-center flex items-center">
                  <button
                    onClick={() => openUserDetail(pharmacists)}
